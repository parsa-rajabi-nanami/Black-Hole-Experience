...
void gasAt(vec3 p, float rd, float dt, out float dens, out vec3 tint, out float heat) {
  ...
  float fA = fract(uTime / WIND_CYCLE);
  float fB = fract(uTime / WIND_CYCLE + 0.5);
  ...
  float prof = smoothstep(0.0, 0.07, rn) * (1.0 - smoothstep(0.45, 1.0, rn)) * pow(uDiskIn / rd, 2.0);
  ...
  for (int k = 0; k < 3; k++) {
    ...
    if (h > 0.965) {
      ...
      col += s * (0.6 + 4.5 * fract(h * 97.0))
        * mix(vec3(0.72, 0.82, 1.0), vec3(1.0, 0.88, 0.72), warm) / pow(2.2, float(k));
    }
  }
  ...
  for (int i = 0; i < MAX_STEPS; i++) {
    ...
    if (rd > uDiskIn && rd < uDiskOut && abs(mid.y) < uThick * 5.0) {
      ...
      if (dens > 0.001) {
        ...
        vec3 shift = mix(vec3(1.0), g > 1.0 ? vec3(0.86, 0.94, 1.14) : vec3(1.15, 0.82, 0.62),
...
export function BlackHoleHeroSection({
  ...
  const props = useRef({ distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
    diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor, midColor, coolColor,
    starBrightness, glow, exposure, vignette, steps, resolution, maxDpr, focus, scrim, scrimStrength, paused });
  const wakeRenderer = useRef<(() => void) | null>(null);

  useEffect(() => {
    props.current = { distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
      diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor, midColor, coolColor,
      starBrightness, glow, exposure, vignette, steps, resolution, maxDpr, focus, scrim, scrimStrength, paused };
    wakeRenderer.current?.();
  }, [distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter, diskThickness,
    diskDensity, brightness, spinSpeed, grain, doppler, hotColor, midColor, coolColor, starBrightness,
    glow, exposure, vignette, steps, resolution, maxDpr, focus, scrim, scrimStrength, paused]);
  ...
  useEffect(() => {
    ...
    const opts = { alpha: false, antialias: false, depth: false, stencil: false, powerPreference: "high-performance", preserveDrawingBuffer: false };
    const context = (canvas.getContext("webgl2", opts) || canvas.getContext("webgl", opts)) as WebGL2RenderingContext | WebGLRenderingContext | null;
    const giveUp = (why: string) => { host.dataset.webgl = why; canvas.style.display = "none"; };
    if (!context) { giveUp("unsupported"); return; }
    const gl = context;
    ...
    const lowPower = software || cores <= 4 || deviceMemory <= 4;
    ...
    function dropTargets() {
      for (const target of [scene, histA, histB, bloomA, bloomB]) {
        if (target) { gl.deleteTexture(target.tex); gl.deleteFramebuffer(target.fb); }
      }
    ...
    function resize() {
      ...
      const dpr = software ? 1 : Math.min(window.devicePixelRatio || 1, lowPower ? 1.25 : Math.max(1, props.current.maxDpr));
      const cssW = Math.max(1, Math.round(rect.width));
      const cssH = Math.max(1, Math.round(rect.height));
      const scale = software ? 0.34 : Math.min(1, Math.max(lowPower ? 0.45 : 0.4, props.current.resolution));
      ...
      const bw = Math.max(2, sw >> 2); const bh = Math.max(2, sh >> 2);
      bloomA = makeTarget(bw, bh); bloomB = makeTarget(bw, bh);
      if (!scene || !histA || !histB || !bloomA || !bloomB) {
        giveUp("allocation-failed");
        running = false;
      }
    }
    ...
    function render(t: number) {
      if (!sceneProg || !blendProg || !brightProg || !blurProg || !compProg || !scene || !histA || !histB || !bloomA || !bloomB) return;
      ...
      const camX = dist * ce * Math.cos(az); const camY = dist * Math.sin(el); const camZ = dist * ce * Math.sin(az);
      const fx = -camX / dist; const fy = -camY / dist; const fz = -camZ / dist;
      let rx = fz; let ry = 0; let rz = -fx;
      const rl = Math.hypot(rx, ry, rz) || 1; rx /= rl; ry /= rl; rz /= rl;
      let ux = ry * fz - rz * fy; let uy = rz * fx - rx * fz; let uz = rx * fy - ry * fx;
      const cr = Math.cos(C.roll * RAD); const sr = Math.sin(C.roll * RAD);
      const RX = rx * cr + ux * sr; const RY = ry * cr + uy * sr; const RZ = rz * cr + uz * sr;
      const UX = -rx * sr + ux * cr; const UY = -ry * sr + uy * cr; const UZ = -rz * sr + uz * cr;
      const hot = hexToLinear(C.hotColor); const mid = hexToLinear(C.midColor); const cool = hexToLinear(C.coolColor);
      const outer = Math.max(C.diskInner + 0.5, C.diskOuter);
      pass(sceneProg, scene);
      const u = sceneProg.u;
      gl.uniform2f(u.uRes!, scene.w, scene.h); gl.uniform1f(u.uTime!, t); gl.uniform3f(u.uCamPos!, camX, camY, camZ);
      gl.uniform3f(u.uRight!, RX, RY, RZ); gl.uniform3f(u.uUp!, UX, UY, UZ); gl.uniform3f(u.uFwd!, fx, fy, fz);
      gl.uniform1f(u.uTanHalf!, Math.tan(Math.max(8, Math.min(110, C.fov)) * 0.5 * RAD));
      gl.uniform2f(u.uFocus!, C.focus[0], 1 - C.focus[1]);
      gl.uniform1f(u.uSteps!, lowPower ? Math.min(220, Math.max(60, Math.round(C.steps))) : Math.max(60, Math.min(460, Math.round(C.steps))));
      gl.uniform1f(u.uSkyR!, Math.max(dist * 1.35, outer * 2.4)); gl.uniform1f(u.uDiskIn!, Math.max(1.05, C.diskInner));
      gl.uniform1f(u.uDiskOut!, outer); gl.uniform1f(u.uThick!, Math.max(0.02, C.diskThickness));
      gl.uniform1f(u.uDensity!, Math.max(0, C.diskDensity)); gl.uniform1f(u.uSpin!, C.spinSpeed * 6.2831853);
      gl.uniform1f(u.uGrain!, Math.max(0.02, C.grain)); gl.uniform1f(u.uBright!, Math.max(0, C.brightness));
      gl.uniform1f(u.uDoppler!, Math.max(0, Math.min(1, C.doppler))); gl.uniform3f(u.uHot!, hot[0], hot[1], hot[2]);
      gl.uniform3f(u.uMid!, mid[0], mid[1], mid[2]); gl.uniform3f(u.uCool!, cool[0], cool[1], cool[2]);
      gl.uniform1f(u.uStars!, Math.max(0, C.starBrightness)); gl.uniform1f(u.uEncode!, hdr ? 0 : 1);
      const h = HALTON[settled % HALTON.length]; gl.uniform2f(u.uJitter!, h[0] - 0.5, h[1] - 0.5); gl.uniform1f(u.uSeed!, (settled % 64) * 17.13); draw();
      ...
      const shown = histB; const temp = histA; histA = histB; histB = temp; settled++;

      pass(brightProg, bloomA); bind(shown.tex, 0); gl.uniform1i(brightProg.u.uTex!, 0);
      gl.uniform2f(brightProg.u.uTexel!, 1 / shown.w, 1 / shown.h); gl.uniform1f(brightProg.u.uDecode!, hdr ? 0 : 1);
      gl.uniform1f(brightProg.u.uPack!, pack); gl.uniform1f(brightProg.u.uThreshold!, 0.85); draw();
      const blurStep = (source: Target, target: Target, dx: number, dy: number) => {
        pass(blurProg!, target); bind(source.tex, 0); gl.uniform1i(blurProg!.u.uTex!, 0); gl.uniform2f(blurProg!.u.uStep!, dx / target.w, dy / target.h); draw();
      };
      blurStep(bloomA, bloomB, 1, 0); blurStep(bloomB, bloomA, 0, 1); blurStep(bloomA, bloomB, 2.6, 0); blurStep(bloomB, bloomA, 0, 2.6);

      pass(compProg, null); bind(shown.tex, 0); bind(bloomA.tex, 1); gl.uniform1i(compProg.u.uScene!, 0); gl.uniform1i(compProg.u.uBloom!, 1);
      ...
      gl.uniform1f(compProg.u.uScrimAmt!, Math.max(0, Math.min(1, C.scrimStrength))); gl.uniform1f(compProg.u.uSeed!, (t * 60) % 1000); draw();
    }

    function settle(passes: number) { for (let i = 0; i < passes; i++) render(clock); }
    function schedule() {
      if (running && visible && !reduced && !props.current.paused && !raf) raf = requestAnimationFrame(tick);
    }
    function tick(now: number) {
      raf = 0;
      if (!running || !visible || props.current.paused) return;
    ...
    schedule();
    const ro = new ResizeObserver(() => { resize(); if (reduced || props.current.paused) settle(16); schedule(); });
    ro.observe(host);
    ...
    io.observe(host);
    const onVisibility = () => { visible = !document.hidden; lastFrame = 0; schedule(); };
    const onLost = (event: Event) => { event.preventDefault(); running = false; cancelAnimationFrame(raf); host.dataset.webgl = "context-lost"; canvas.style.display = "none"; };
    const onRestored = () => { width = 0; height = 0; sceneW = 0; sceneH = 0; if (!build()) { giveUp("lost"); return; } canvas.style.display = ""; delete host.dataset.webgl; running = true; resize(); lastFrame = 0; settle(reduced ? 16 : 1); schedule(); };
    document.addEventListener("visibilitychange", onVisibility);
    canvas.addEventListener("webglcontextlost", onLost);
    canvas.addEventListener("webglcontextrestored", onRestored);
    return () => {
      running = false; wakeRenderer.current = null; cancelAnimationFrame(raf); ro.disconnect(); io.disconnect(); document.removeEventListener("visibilitychange", onVisibility);
      canvas.removeEventListener("webglcontextlost", onLost); canvas.removeEventListener("webglcontextrestored", onRestored); dropTargets();
      if (vbo) gl.deleteBuffer(vbo);
      for (const program of [sceneProg, blendProg, brightProg, blurProg, compProg]) if (program) gl.deleteProgram(program.program);
    };
  }, []);
